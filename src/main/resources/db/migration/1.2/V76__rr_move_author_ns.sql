alter table reading_room_contributors drop column name_ns;

alter table reading_room_object_contributors add column name_ns varchar(255);
alter table reading_room_part_contributors add column name_ns varchar(255);
alter table reading_room_content_contributors add column name_ns varchar(255);
...
select 
    ...
    group_concat(distinct ifnull(p.script, p.script_imported) order by p.script separator '; ') as scripts,
    group_concat(distinct ifnull(p.writing_system, p.writing_system_imported) order by p.writing_system separator '; ') as writing_systems,
    ...
    group_concat(DISTINCT scribes.name_ns SEPARATOR '; ') as scribes_ns,
