CREATE INDEX reading_room_object_contrib_idx1 ON reading_room_object_contributors(contributor_id);
CREATE INDEX reading_room_object_contrib_idx2 ON reading_room_object_contributors(reading_room_object_id);
CREATE INDEX reading_room_part_contrib_idx1 ON reading_room_part_contributors(part_id);
CREATE INDEX reading_room_part_contrib_idx2 ON reading_room_part_contributors(contributor_id);
CREATE INDEX reading_room_object_features_idx1 ON reading_room_object_features(feature_id);
CREATE INDEX reading_room_object_features_idx2 ON reading_room_object_features(reading_room_object_id);

create or replace view reading_room_former_owners as
select 
	reading_room_object_id, 
    contributor_id 
from 
	reading_room_object_contributors 
where 
	type = 'FORMER_OWNER';
	
create or replace view reading_room_scribes as
select 
	part_id, 
    contributor_id 
from 
...
where 
	contributor_type = 'SCRIBE';
	
create or replace view reading_room_artists as
select 
	part_id, 
    contributor_id 
from 
