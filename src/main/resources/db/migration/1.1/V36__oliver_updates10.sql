CREATE INDEX object_extents_idx1 ON reading_room_object_extents(reading_room_object_id);

CREATE INDEX alt_titles_idx1 ON reading_room_alternate_titles(content_id);

CREATE INDEX content_u_titles_idx1 ON reading_room_content_uniform_titles(content_id);

CREATE INDEX content_u_titles_idx2 ON reading_room_content_uniform_titles(title_id);

CREATE INDEX content_contrib_idx1 ON reading_room_content_contributors(contributor_type);

create or replace view reading_room_authors as
select 
...
from reading_room_objects o left outer join
    ...
    reading_room_authors authors on cnt.id = authors.reading_room_content_id left outer join
