CREATE INDEX reading_room_objects_idx3 ON reading_room_objects(archival_data_id);
CREATE INDEX archival_data_languages_idx1 ON reading_room_archival_data_languages(archival_data_id);

alter table reading_room_archival_data drop column reading_room_object_id;

create or replace view reading_room_archival_associated_names as
select 
	archival_data_id, 
    contributor_id,
    name_ns
from 
	reading_room_archival_contributors 
where 
	contributor_type is null;
	
create or replace view reading_room_content_artists as	
select 
...
create or replace view reading_room_content_scribes as	
select 
	reading_room_content_id, 
    reading_room_contributor_id,
    name_ns
