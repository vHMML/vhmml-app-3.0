...
public class ImageController {
	...
	/**
	 * This method will get the project manifest using an object type.  A typical URL for this would be
	 * "/image/manifest/folio/12345".
	 * 
	 * @param objectId
	 * @param objectType
	 * @param response
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value = "/manifest/{objectType}/{objectId}", method = RequestMethod.GET)
	public void getProjectManifestWithObjectType(@PathVariable Long objectId, @PathVariable String objectType, HttpServletResponse response, HttpServletRequest request) throws IOException {
		// If the object type is "folio" and manifest access is not allowed, then
		// we want to send a 403 error and message telling the user that permission is not allowed.
		if ("folio".equalsIgnoreCase(objectType) && !checkManifestAccess(objectId)) {
			response.setStatus(HttpServletResponse.SC_FORBIDDEN);
			response.getWriter().write("You do not have permission to access the requested manifest");
		} else {
			getProjectManifest(objectId, objectType, response, request);
		}
	}
	...
	/**
	 * This method will check the manifest access for FOLIO objects because the manifest URL FOLIO objects
	 * will be restricted.
	 * 
	 * @param objectType
	 * @param objectId
	 * @return
	 */
	private boolean checkManifestAccess(Long objectId) {
		return folioService.checkManifestAccess(objectId);
	}
	
	/**
	 * This method will check the image access using the object type and object id.  It will check for FOLIO type first
	 * then RR type.
	 * @param objectType
	 * @param objectId
	 * @return
	...
	/**
	 * This method will check the image access using the object type and project number.  It will check for FOLIO type first
	 * then RR type.
	 * @param objectType
	 * @param projectNumber
	 * @return
