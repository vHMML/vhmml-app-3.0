...
public class UserController extends BaseValidationController {
	...
	public ModelAndView saveAccountSettings(@ModelAttribute @Valid RegistrationForm registrationForm, BindingResult result, HttpServletRequest request) throws IOException {
		...
		if (registrationForm.getEnabled() == null) {
			registrationForm.setEnabled(user.getEnabled());
		}
		
		ModelAndView successModelAndView = homeController.index(request);
		return userControllerHelper.saveAccountSettings(registrationForm, result, successModelAndView, true);			
	}	
	...
	public ModelAndView changePassword(@ModelAttribute @Valid ChangePasswordForm changePasswordForm, BindingResult result, HttpServletRequest request) throws IOException {
		...
		if(!errors) {
			try {
				userService.processChangePasswordRequest(changePasswordForm, request);
	...
	public ModelAndView submitForgotPassword(@ModelAttribute @Valid ForgotPasswordForm forgotPasswordForm, BindingResult result, HttpServletRequest request) throws IOException {
		ModelAndView modelAndView = new ModelAndView(VIEW_FORGOT_PASSWORD);
		boolean errors = result.hasErrors();
		
		if(!errors) {
			String email = forgotPasswordForm.getEmailAddress();
			User user = userService.findByUsername(email);
			Integer failedLogins = user.getFailedLogins();
			
			if(failedLogins != null && failedLogins >= configService.getIntValue(Property.MAX_FAILED_LOGINS)) {
				VhmmlMessage message = new VhmmlMessage("Your account is locked. Please contact help@vhmml.org to reset your account.", Severity.ERROR);				
				modelAndView.addObject(ControllerConstants.REQUEST_ATT_PAGE_MESSAGES, objectMapper.writeValueAsString(message));
				errors = true;
			} else {
				try {				
