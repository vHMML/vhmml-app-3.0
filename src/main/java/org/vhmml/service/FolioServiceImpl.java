...
import org.vhmml.service.ReadingRoomServiceImpl.SearchField;
...
public class FolioServiceImpl implements FolioService {
				...
				switch(field) {
					case "keyword":
						...
						if(exactSearch) {
							...
							keywordQuery.must(new MultiMatchQuery(searchString, SearchField.getAllFields(), MultiMatchQueryBuilder.Type.PHRASE, 0));
							...
							keywordQuery.should(new WildcardQuery(searchString, SearchField.getAllFields()));
							...
							keywordQuery.should(new MultiMatchQuery(strippedSeachString, SearchField.getBasicFields(), MultiMatchQueryBuilder.Type.PHRASE, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.SHOULD));
							keywordQuery.setFields(SearchField.getAllFields());
						} else {
							...
							if (searchStringTokens.length > 1) {
								keywordQuery.should(new MultiMatchQuery(searchString, SearchField.getBasicFields(), MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.SHOULD));
							} else {
								keywordQuery.must(new MultiMatchQuery(searchString, SearchField.getBasicFields(), MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.MUST));
							}
							
							keywordQuery.setFields(SearchField.getBasicFields());
						}
	...
	public boolean checkManifestAccess(Long projectId) {
		FolioObject object = find(projectId);		
		return object.getPublicManifest();
	}
}
