...
//				partsByObjectId = importReadingRoomObjectParts(rs, importJob, contentByPartId, imagesByContentId);
				partsByObjectId = importReadingRoomObjectParts(rs, importJob, contentByPartId, null);
			}			
...
//					readingRoomObject.setFoliation(StringUtils.trim(rs.getString("Foliation")));
			...
			String genreString = StringUtils.trim(rs.getString("GenreForm"));
			...
			if(StringUtils.isNotEmpty(genreString)) {
				List<Genre> genresForObject = findOrCreateGenres(genreString);
				
				for(Genre genre : genresForObject) {
					genres.add(new ReadingRoomObjectGenre(id, genre.getId()));
				}
	...
	private Map<Long, List<ReadingRoomObjectPart>> importReadingRoomObjectParts(ResultSet rs, Import importResult, Map<Long, List<Content>> contentByPart, Map<Long, List<Image>> imagesByContent) throws Exception {
		...
		while(rs.next()) {
			...
			String scriptString = StringUtils.trim(rs.getString("Script"));
			
			readingRoomObjectPart.setScriptImported(scriptString);
			
			if(StringUtils.isNotEmpty(scriptString)) {
				findOrCreateScripts(scriptString);
				readingRoomObjectPart.setScript(scriptString.replaceAll(";", ","));
			}
			...
			String writingSystemString = StringUtils.trim(rs.getString("Alphabet"));
			
			readingRoomObjectPart.setWritingSystemImported(writingSystemString);
			
			if(StringUtils.isNotEmpty(writingSystemString)) {
				findOrCreateWritingSystems(writingSystemString);
				readingRoomObjectPart.setWritingSystem(writingSystemString.replaceAll(";", ","));
			}	
		...
		while(rs.next()) {
			...
			if(CollectionUtils.isNotEmpty(otherContributors)) {
				for(Contributor contributor : otherContributors) {
					contentContributors.add(new ContentContributor(content, contributor, null, null));
				}				
			}
	...
	private List<WritingSystem> findOrCreateWritingSystems(String writingSystemString) {
	...
	private List<Script> findOrCreateScripts(String scriptString) {
		List<Script> scripts = new ArrayList<Script>();
		
		if(StringUtils.isNotEmpty(scriptString)) {
			String[] scriptNames = scriptString.split(";");
			
			for(String scriptName : scriptNames) {
				scriptName = scriptName.trim();
				Script script = scriptCache.get(scriptName);
				
				if(script == null && StringUtils.isNotEmpty(scriptName)) {
					
					script = scriptRepository.findByName(scriptName);
					
					if(script == null) {
						script = scriptRepository.save(new Script(scriptName));
					}
	...
	private List<Genre> findOrCreateGenres(String genreString) {
	...
	private ArchivalData getArchivalData(ResultSet rs) throws SQLException, IOException, ParseException {
		Long id = rs.getLong("MsID");
		...
		String centuryDate = StringUtils.trim(rs.getString("ArcDateCentury"));
		
		if(!"Unknown".equalsIgnoreCase(centuryDate)) {
			archivalData.setCenturyImported(centuryDate);
		}
		...
		if(StringUtils.isNotEmpty(monthString)) {
			if(NumberUtils.isNumber(monthString)) {
				...
				cal.setTime(date);
				archivalData.setDatePreciseMonth(cal.get(Calendar.MONTH));
			}			
		}
		...
		if(StringUtils.isNotEmpty(beginDateString) && NumberUtils.isNumber(beginDateString)) {
			archivalData.setBeginDate(Integer.valueOf(beginDateString));			
		}
		
		if(StringUtils.isNotEmpty(endDateString) && NumberUtils.isNumber(beginDateString)) {
	...
	private List<ArchivalContributor> getArchivalContributors(ResultSet rs) throws SQLException, IOException {
		...
		String[] associatedNameTypes = StringUtils.isNotEmpty(associatedNameType) ? StringUtils.trim(associatedNameType).split(";") : new String[] {""};
		...
		for(int i = 0; i < contributors.size(); i++) {
			...
			try {
				if(StringUtils.isNotEmpty(contributorType)) {
					Contributor.Type type = Contributor.Type.forDisplayName(contributorType);
					archivalContributor.setType(type);
				}				
			} catch(IllegalArgumentException e) {
				LOG.error("Invalid contributor type on archival record being imported with id " + id + ", contributor type will be empty.", e);
				archivalContributor.setType(null);
			}
