...
import org.vhmml.entity.Role.Name;
...
//			params.put("description", shelfMark);
	...
	public String getExternalIIIFData(String url) throws IOException {		
		...
		if(StringUtils.isNotEmpty(url)) {			
			try {
				jsonData = remoteServiceUtil.executeExternalGet(url, null, true);
				...
				LOG.error("Unexpected error making remote call to get external IIIF data at URL " + url, e);
			}		
		}		
	...
	public void trackImageViewingData(ReadingRoomObject object) {
	...
	/**
	 * Method will determine if a user should be tracked for the image_view report based on the roles listed.
	 * 
	 * @param user
	 * @return
	 */
	private boolean shouldTrackUser(User user) {
		ArrayList<Name> roleExclusionList = Lists.newArrayList(
				Role.Name.ROLE_ADMIN,
				Role.Name.ROLE_LEVEL_I_CATALOGER, 
				Role.Name.ROLE_LEVEL_II_CATALOGER, 
				Role.Name.ROLE_LEVEL_III_CATALOGER);
		
		// return false if the user has a matching role to the above role list
		return !user.hasRole(roleExclusionList);
	}
