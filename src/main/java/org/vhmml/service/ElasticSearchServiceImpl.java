...
public class ElasticSearchServiceImpl implements ElasticSearchService {
	...
	private static enum StopWordFilter {
		// Other default stopword filters that can be included are..
		// _french_, _italian_, _german_, etc.
	...
	private static enum CustomStopWordFilter {
		CUSTOM_STOPWORDS("custom_stopwords", "custom_stopwords.txt");
		
		private String name;
		private String customStopWordFile;
		
		CustomStopWordFilter(String name, String customStopWordFile) {
			this.name = name;
			this.customStopWordFile = customStopWordFile;
		}
		
		static List<String> getFilterNames() {
			List<String> filterNames = new ArrayList<String>();
			
			for(CustomStopWordFilter filter : CustomStopWordFilter.values()) {
				filterNames.add(filter.name);
			}
	...
	public SearchResponse multiFieldSearch(IndexedType indexedType, List<ElasticSearchQuery> queries, QueryBuilder filter, boolean highlightHits, Pageable pageable, DateRangeFilter dateRangeFilter) {
		...
		for(ElasticSearchQuery query : queries) {
			...
			if(highlightHits) {
				...
				} else if (BoolQuery.class.isAssignableFrom(queryClass)) {
					String[] fields = ((BoolQuery)query).getFields();
					for (String field : fields) {
						hitHighlightFieldsMap.put(field, field);
					}
				}
		...
		if(filter != null) {
			boolQuery = addQueryFilter(boolQuery, filter);
		}
	...
	public SearchResponse multiFieldSearch(IndexedType indexedType, List<ElasticSearchQuery> queries, boolean highlightHits, Pageable pageable, DateRangeFilter dateRangeFilter) {
		return multiFieldSearch(indexedType, queries, null, highlightHits, pageable, dateRangeFilter);
	}
	...
	public SearchResponse multiFieldSearch(IndexedType indexedType, List<ElasticSearchQuery> queries, boolean highlightHits, Pageable pageable) {		
		return multiFieldSearch(indexedType, queries, null, highlightHits, pageable, null);
	}	
	
	public SearchResponse multiFieldSearch(IndexedType indexedType, List<ElasticSearchQuery> queries, QueryBuilder filter, boolean highlightHits, Pageable pageable) {		
		return multiFieldSearch(indexedType, queries, filter, highlightHits, pageable, null);
	}	
	...
	public SearchResponse fuzzySearch(IndexedType indexedType, String searchText, Pageable pageable, String... fields) {
		...
		QueryBuilder fuzzyQuery = QueryBuilders.multiMatchQuery(searchText, fields).fuzziness(Fuzziness.fromEdits(fuzziness));
	...
	public QueryBuilder addQueryFilter(QueryBuilder queryBuilder, QueryBuilder queryFilter) {
		QueryBuilder query = queryBuilder;
		
		if(queryFilter != null) {
			query = QueryBuilders.boolQuery().must(queryBuilder).filter(queryFilter);
		}
	...
	public QueryBuilder addQueryDateFilter(QueryBuilder queryBuilder, DateRangeFilter dateRangeFilter) {
		...
		if(dateRangeFilter != null) {
			...
			RangeQueryBuilder dateFilter = new RangeQueryBuilder(dateRangeFilter.getDateField()).gte(startDate).lte(endDate);
			query = QueryBuilders.boolQuery().must(queryBuilder).filter(dateFilter);
		}
		...
		if(pageable != null) {
			...
			if(sort != null) {
				...
				while(sortFields.hasNext()) {
					...
					// Check to see if the sort field is the My List custom "Date Saved" field.  If it is then we want to skip putting together
					// the sort field for Elastic Search since "Date Saved" is not indexed and won't be indexed in ES.  The field 
					// is in the my_list_items table, tied to a user id AND object id.
					if (!sortField.getProperty().equals(MY_LIST_CUSTOM_SORT_DATESAVED)) {
						...
						boolean indexContainsDateTypeField = VhmmlIndexFields.containsDateTypeField(indexFields);
						String sortProperty;
						
						// Check to see if the index contains "date", and if the "date" datatype is in the list of fields, AND if the
						// sort field is "date", we want to assign the property instead of the raw version.  Date datatypes do not have
						// raw, untouched, etc. values.
						if (indexContainsSortField && indexContainsDateTypeField && ("date").equalsIgnoreCase(sortField.getProperty())) {
							sortProperty = sortField.getProperty();
						} else {
							sortProperty = indexContainsSortField ? sortField.getProperty() + ".raw" : sortField.getProperty();
						}
	...
	private Client getClient(String clusterName) {
		...
		return transportClient.addTransportAddress(new InetSocketTransportAddress(new InetSocketAddress(elasticSearchHost, elasticSearchPort)));
	}
	...
	/**
	 * Method will add the custom stopword filters to the index settings.
	 * @param mapping
	 * @param customStopWordFilter
	 * @throws IOException
	 */
	private void addCustomStopWordFilter(XContentBuilder mapping, CustomStopWordFilter customStopWordFilter) throws IOException {
		mapping.startObject(customStopWordFilter.name)
				.field("type", "stop")
				.field("stopwords_path", customStopWordFilter.customStopWordFile)
			.endObject();
	}
	...
	private XContentBuilder getIndexSettings() {
		...
		foldingAnalyzerFilters.addAll(CustomStopWordFilter.getFilterNames());
		...
		defaultAnalyzerFilters.addAll(CustomStopWordFilter.getFilterNames());
				...
				.startObject()
					.startObject("analysis")
						...
						.startObject("filter");
							...
							for(CustomStopWordFilter filter : CustomStopWordFilter.values()) {
								addCustomStopWordFilter(mapping, filter);
							}
	...
	private XContentBuilder addIndexedFieldMapping(XContentBuilder mapping, IndexedField field) throws IOException {
		...
		mapping.startObject(field.getName())
			...
			// Only proceed with the rest of the fields for a field type if it is NOT a
			// date datatype and the format value is empty.  
			if (!field.getType().equals("date") && StringUtils.isEmpty(field.getFormat())) {
				...
				.startObject()
					.startObject("properties");
						for(IndexedField field : fields) {
							mapping = addIndexedFieldMapping(mapping, field);							
						}						
					mapping.endObject();
				mapping.endObject();
		} catch (IOException e) {
