...
public class ReadingRoomServiceImpl implements ReadingRoomService {	
	...
	protected enum SearchField {	
		...
		CONTENTS_DETAIL("Contents Detail", "title", new String[]{"contentsDetail"}),		
		...
		ALTERNATE_SURROGATES("Alternate Surrogates", "hmmlProjectNumber", new String[]{"alternateSurrogates", "alternateSurrogates.zero_trim", "alternateSurrogates.zero_trim_folded"}, false),
		...
		ARCHIVAL_COLLECTION_FOND("Archival Collection/Fonds", "archivalCollectionFond", new String[]{"archivalCollectionFond"}),		
		ARCHIVAL_SERIES("Archival Series", "archivalSeries", new String[]{"archivalSeries"}),		
		ARCHIVAL_SUB_SERIES("Archival Sub-series", "archivalSubSeries", new String[]{"archivalSubSeries"}),		
		ARCHIVAL_SUB_SUB_SERIES("Sub-sub-series", "archivalSubSubSeries", new String[]{"archivalSubSubSeries"}),
		...
		EXTERNAL_BIBLOGRAPHY("External Biblography", "externalbiblography", new String[]{"externalBiblography"}),		
		PRINTING_STATEMENT("Printing Statement", "printingStatement", new String[]{"printingStatement"});		
	...
	public List<ReadingRoomObject> getAllObjectsByExportOption(String exportOption) {
		if (ACTIVE_ONLY.equals(exportOption)) {
			return Lists.newArrayList(readingRoomObjectRepository.findByActive(true));
		} else if (INACTIVE_ONLY.equals(exportOption)) {
			return Lists.newArrayList(readingRoomObjectRepository.findByActive(false));
		} else {
			return Lists.newArrayList(readingRoomObjectRepository.findAll());
		}
	}
	...
	public List<ReadingRoomObject> findByCollection(String collectionName, String exportOption) {
		// Check the export option and then call the repo method passing in the appropriate parameter.
		if (ACTIVE_ONLY.equals(exportOption)) {
			return readingRoomObjectRepository.findByActiveAndHmmlProjectNumberStartsWith(true, collectionName);
		} else if (INACTIVE_ONLY.equals(exportOption)) {
			return readingRoomObjectRepository.findByActiveAndHmmlProjectNumberStartsWith(false, collectionName);
		} else {
			...
			if(archivalData != null) {
				...
				if(archivalData != null && CollectionUtils.isNotEmpty(contentList)) {
					...
					for(ArchivalContent content : contentList) {
						...
						content.setAssociatedNames(content.getContributorsExcludingTypes(Contributor.Type.SCRIBE));
	...
	public ReadingRoomObject saveReadingRoomObject(ReadingRoomObject object) throws IOException {		
		...
		object.setSupport(getSupportList(object));
	...
	public void deleteReadingRoomObjectPart(Long partId) throws IllegalAccessException, IOException {
		...
		if(part != null) {
			...
			if(currentUserCanEdit(parentObject)) {
				...
				parentObject.setSupport(getSupportList(parentObject));				
	...
	/**
	 * Method will perform a search for RR records where the flag for active records only is passed in as a parameter.
	 * @param searchTerms
	 * @param pageable
	 ...
	 */
	@Override
	public ReadingRoomSearchResult search(Map<String, String> searchTerms, Pageable pageable, boolean activeRecordsOnly, boolean highlightHits) {
		return this.doSearch(searchTerms, pageable, activeRecordsOnly, highlightHits);
	}
	
	/**
	 * Method will perform a search for RR records where the flag for active records only is set to false by default, and highlighting set to true by default.
	 * This method is for the default behavior of the RR search.
	 */
	@Override
	public ReadingRoomSearchResult search(Map<String, String> searchTerms, Pageable pageable) {
		return this.doSearch(searchTerms, pageable, false, true);
	}
	
	public ReadingRoomSearchResult doSearch(Map<String, String> searchTerms, Pageable pageable, boolean activeRecordsOnly, boolean highlightHits) {		
		
		List<ElasticSearchQuery> elasticSearchQueries = new ArrayList<>();
		QueryBuilder searchQueryFilter = null;
		boolean isCataloger = VhmmlSecurityUtil.isCurrentUserCataloger();
		boolean isScholar = VhmmlSecurityUtil.isCurrentScholar();
				
		// Check if the flag for retrieving active records is set.  If it is then we want to pull ONLY active records, no matter
		// who the user is.  Example use: data portal search.
		if (activeRecordsOnly) {
			searchQueryFilter = QueryBuilders.boolQuery().must(QueryBuilders.termQuery("active", true));
		} else {
			...
			if(isScholar && !isCataloger) {
				searchQueryFilter = QueryBuilders.boolQuery()
					.should(QueryBuilders.termQuery("active", true))
					.should(QueryBuilders.termQuery("assignedScholar", VhmmlSecurityUtil.getCurrentUser().getId().toString()));
			}
			...
			if(StringUtils.isNotEmpty(searchString)) {
				...
				switch(field) {
					case "keyword":
						...
						BoolQuery keywordQuery = new BoolQuery();
						
						if(exactSearch) {
							// Add the multimatch query to the bool query as a MUST because it is for exact search.
							keywordQuery.must(new MultiMatchQuery(searchString, SearchField.getKeywordExactMatchFields(), MultiMatchQueryBuilder.Type.PHRASE, 0));
							// Set the fields in the bool query so that we can use it for highlighting the fields.
							keywordQuery.setFields(SearchField.getKeywordExactMatchFields());
						} else if(wildcardSearch) {
							keywordQuery.should(new WildcardQuery(searchString, SearchField.getKeywordFields(), org.elasticsearch.index.query.QueryStringQueryBuilder.Operator.AND));
							...
							String strippedSearchString = searchString.replaceAll("\\*", "").replaceAll("\\?", "");
							keywordQuery.should(new MultiMatchQuery(strippedSearchString, SearchField.getKeywordExactMatchFields(), MultiMatchQueryBuilder.Type.PHRASE, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.SHOULD));
							keywordQuery.setFields(SearchField.getAllFields());
						} else {
							...
							if (searchStringTokens.length > 1) {
								keywordQuery.should(new MultiMatchQuery(searchString, SearchField.getKeywordFields(), MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.SHOULD));
							} else {
								keywordQuery.must(new MultiMatchQuery(searchString, SearchField.getKeywordFields(), MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.MUST));
							}
							
							keywordQuery.setFields(SearchField.getKeywordFields());
						}
						
						elasticSearchQueries.add(keywordQuery);
												
						break;
					case "beginDate":
						MultipleFieldRangeQuery rangeQuery = new MultipleFieldRangeQuery(searchTerms.get("endDate"), searchString, new String[] {"beginDate", "endDate"});
						
						QueryBuilder dateRangeFilter = QueryBuilders.boolQuery().filter(rangeQuery.getQuery());
						
						// Check to see if the searchQueryFilter is null, if it isn't then we want to tack on the date range "filter".
						// If it is null, then we assign the date range "filter" directly to the variable.
						if (searchQueryFilter != null) {
							searchQueryFilter = ((BoolQueryBuilder)searchQueryFilter).filter(dateRangeFilter);
						} else {
							searchQueryFilter = dateRangeFilter;
						}
					...
					case "hmmlProjectNumber":
						...
						elasticSearchQueries.add(new MultiMatchQuery(searchString, zeroTrimFields, MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.MUST));																		
						break;
					case "author":
					case "title":
					case "incipit":
						boolean isExactSearch = searchString.startsWith("\"") && searchString.endsWith("\"");
						BoolQuery query = new BoolQuery();
						
						if(isExactSearch) {
							// Add the multimatch query to the bool query as a MUST because it is for exact search.
							query.must(new MultiMatchQuery(searchString, SearchField.getFieldsForGroup(field), MultiMatchQueryBuilder.Type.PHRASE, 0));
							// Set the fields in the bool query so that we can use it for highlighting the fields.
							query.setFields(SearchField.getKeywordFields());
						} else {
							query.must(new MultiMatchQuery(searchString, SearchField.getFieldsForGroup(field), MultiMatchQueryBuilder.Type.CROSS_FIELDS, org.elasticsearch.index.query.MatchQueryBuilder.Operator.AND, BoolOperation.MUST));
							query.setFields(SearchField.getKeywordFields());
						}
						
						elasticSearchQueries.add(query);
	...
	private boolean checkImageAccess(ReadingRoomObject object) {
		...
		try {			
			...
			if(object != null) {
				...
				// If the access restriction is REGISTERED or REGISTERED_ONLY, then the user only has access if they are a registered user, not anonymous.
				if (accessRestriction.equals(AccessRestriction.REGISTERED) || accessRestriction.equals(AccessRestriction.REGISTERED_ONLY)) {
					hasAccess = principal instanceof User;
				} else {
					// If the access restriction is UNREGISTERED then the user should have access to it.
					hasAccess = accessRestriction.equals(AccessRestriction.UNREGISTERED);
				}
	...
	/**
	 * Method will return search results for Reading Room.  This method is also called by the My List service.
	 * @param searchResponse
	 * @param pageable
	 * @return
	 */
	@Override
	public ReadingRoomSearchResult getSearchResult(SearchResponse searchResponse, Pageable pageable) {
        ...
        for(SearchHit hit : hits) {
        	...
        	readingRoomObjectOverview.setPrintingStatement((String)source.get("printingStatement"));   
	...
	private String getSupportList(ReadingRoomObject object) {
		...
		if(object instanceof ArchivalObject) {
			...
			if(archivalData != null) {
				supportList = archivalData.getSupportDisplay();
			}
		} else {
			supportList = getSupportString(getSupports(object.getParts()));
		}
		
		return supportList;
	}
	
	private Map<String, Support> getSupports(List<ReadingRoomObjectPart> parts) {		
	...
	private String getSupportString(Map<String, Support> supports) {
