...
public class ReadingRoomObjectPart implements Identifiable {
	...
	public String getDatePreciseDisplay() {
		String datePreciseDisplay = AttributeUtil.getDatePreciseDisplay(this.datePreciseYear, this.datePreciseMonth, this.datePreciseDay);
		
		if(StringUtils.isEmpty(datePreciseDisplay)) {
			datePreciseDisplay = getYmdDateImported();
		}
		
		return datePreciseDisplay;		
	}
	...
	public List<ReadingRoomPartContributor> getAssociatedNames() {
		...
		if(CollectionUtils.isNotEmpty(contributors)) {
			for(ReadingRoomPartContributor contributor : contributors) {
				...
				if(Contributor.Type.SCRIBE != type) {
