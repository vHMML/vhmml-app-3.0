		...
		if(itemType != null) {
			...
			if(StringUtils.isNotEmpty(authors)) {
				...
				for(String author : authorList) {
					html.append(getCoinsAttribute("rft.au", author));
				}
			}
			
			if(StringUtils.isNotEmpty(otherContributors)) {
				String[] contributorList = otherContributors.split(";");
				
				for(String contributor : contributorList) {
					html.append(getCoinsAttribute("rft.au", contributor.trim()));
				}
			}
