...
$(function() {
	...
	$('div.readingRoomSearchControl textarea, div.readingRoomSearchControl input, div.folioSearchControl textarea, div.folioSearchControl input').on('keypress', function(e) {
		if(e.which === 13) {				
			newSearch();
			return false;
		}
...
function slidePanel($menu) {
	...
	// Check if the menu is the left menu.
	var isLeftMenu = $menu.hasClass('slideOutMenuLeft');
	// There are 2 searchToggleBar elements, one is for the search panel and the other for the metadata panel.
...
function refreshImageViewer(object) {
	...
	// Clos the search panel if it is opened.
