...
$(function() {		
	...
	$('#clearSearchOptionsButton').click(function() {
	...
	if(searchResult && searchResult.items && searchResult.items.length) {			
		...
		for(var i = 0; i < items.length; i++) {
			...
			html += '<tr id="' + item.id +'" data-title="' + item.displayTitle + '">';
				...
				html += '<td class="col-md-2">' + (item.authors ? item.authors : '' );
				...
				if(isReferenceAdmin) {
					html += '<td class="col-md-2 actions text-right">';									
						html += '<a href="' + contextPath + '/reference/admin/edit/' + item.id + '" class="btn btn-primary" role="button">Edit</a>';
						html += '<button onclick="javascript:deleteReferenceEntry(this);" data-entry-id="' + item.id + '" class="btn btn-danger deleteButton" role="button">Delete</button>';
					html += '</td>';					
				}				
			html += '</tr>';			
		}
...
function getTableHeaderHtml() {
	...
	var html = 
		'<caption class="small text-left">' +
			...
			'<a data-toggle="tooltip" data-placement="top" title="Corrections or additions?" href="javascript:showCorrectionsDialog();" class="right"><img src="' + contextPath + '/static/img/glyphicons-conversation2.png" width="20" alt="Corrections or additions?" />&nbsp;Corrections or additions?</a>' +
		'</caption>' +
		
		'<tr>' +
			'<th class="col-md-4"><a href="javascript:sort(\'authors\');">AUTHOR(S)' + (pagingData.sortBy === 'authors' ? sortIcon : '') + '</a></th>' +
