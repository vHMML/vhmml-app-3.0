...
function initTextFilters() {
	...
	filterMaxValue($('input.maxValue'));
	
	$container.arrive('input.digitsOnly', function(){
		$(this).textinput({'filter': 'digits'}); 			
	});
	
	$container.arrive('input.integersOnly', function(){
		$(this).textinput({'whitelist': '0123456789-'}); 			
	});
	
	$container.arrive('input.positiveDecimalsOnly', function(){
		$(this).textinput({'whitelist': '0123456789.'}); 			
	});
	
	$container.arrive('input.maxValue', function() {
		filterMaxValue($(this));
	});
}

function filterMaxValue($inputs) {
	
	$inputs.each(function() {
		var $input = $(this);
		var max = $input.attr('data-max-value');		
					
		$input.change(function() {
			var value = $input.val();
			
			if(max && value !== max) {
				var indexOfDecimal = max.lastIndexOf('.');
				
				if(indexOfDecimal > 0) {
					numberOfDecimals = max.substring(indexOfDecimal + 1).length;
					$input.val(value.substring(0, value.indexOf('.') + numberOfDecimals + 1));
				}
