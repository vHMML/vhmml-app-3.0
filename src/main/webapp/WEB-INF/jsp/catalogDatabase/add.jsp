...
<form:form commandName="readingRoomObjectForm" cssClass="form-horizontal" data-object-type="READING_ROOM" method="POST" action="${pageContext.request.contextPath}/catalogDatabase/save" data-validation-rule-url="/catalogDatabase/readingRoomObject/validationRules">
		...
		<div id="externalFacsimileUrls" class="repeatableField">
			<c:forEach begin="0" end="${vhmmlfn:getFieldCount(readingRoomObjectForm.externalFacsimileUrls)}" varStatus="loopStatus">
				<div class="repeatableFieldGroup groupedFields">
					<vhmml:input label="External Facsimile" listName="externalFacsimileUrls" listIndex="${loopStatus.index}" fieldName="linkText" textarea="true" cssClass="ckEditor" repeatable="true"/>
	...
	<div class="formSection objectData">
		...
		<vhmml:select label="Country" optionDependentListProperty="city" options="${countries}" fieldName="countryId" cssClass="country updateAuthorityFields" addButtonType="COUNTRY" optionLabelProperty="name" optionValueProperty="id" required="true" isAuthorityListField="true"/>				
		<vhmml:select label="City" optionDependentListProperty="repository" options="${citiesByCountry.get(readingRoomObjectForm.countryId)}" fieldName="cityId" cssClass="city updateAuthorityFields" optionValueProperty="id" optionLabelProperty="name" addButtonType="CITY" required="true" isAuthorityListField="true"/>
		...
		<vhmml:hidden fieldName="archivalData.centuryImported" fieldValue="${readingRoomObjectForm.archivalData.centuryImported}" />		
	...
	<div class="formSection objectData">
		...
		<vhmml:dimension-input label="Binding Dimensions" fieldName="binding" helpText="The outer dimensions of the binding measured in centimeters. Round to next closest tenth of a cm. Example: 32.2 x 25 x 3.7 cm." helpIconClass="catalog-database-icon" includeDepth="true" cssClass="positiveDecimalsOnly" maxValue="9999.99" importedValue="bindingDimensionsImported"/>		
		<vhmml:input label="Provenance" fieldName="provenance" textarea="true" cssClass="ckEditor"/>
		
		<%-- level 3 catalogers can't edit former owners, but we don't want to clear the field out when they save the record so we need to send up the value in this hidden field --%>
		<security:authorize ifNotGranted="ROLE_ADMIN">
			<c:forEach begin="0" end="${vhmmlfn:getFieldCount(readingRoomObjectForm.formerOwners)}" varStatus="loopStatus">
				<vhmml:hidden listName="formerOwners" listIndex="${loopStatus.index}" fieldName="name" fieldValue="${readingRoomObjectForm.formerOwners[loopStatus.index].name}"/>
				<vhmml:hidden listName="formerOwners" listIndex="${loopStatus.index}" fieldName="displayName" fieldValue="${readingRoomObjectForm.formerOwners[loopStatus.index].displayName}"/>
				<vhmml:hidden listName="formerOwners" listIndex="${loopStatus.index}" fieldName="authorityUriLC" fieldValue="${readingRoomObjectForm.formerOwners[loopStatus.index].authorityUriLC}"/>
				<vhmml:hidden listName="formerOwners" listIndex="${loopStatus.index}" fieldName="authorityUriVIAF" fieldValue="${readingRoomObjectForm.formerOwners[loopStatus.index].authorityUriVIAF}"/>
			</c:forEach>			
		</security:authorize>
		...
		<security:authorize ifAnyGranted="ROLE_ADMIN">
			<div id="formerOwners" class="repeatableField">
				<c:forEach begin="0" end="${vhmmlfn:getFieldCount(readingRoomObjectForm.formerOwners)}" varStatus="loopStatus">
					<div class="repeatableFieldGroup groupedFields authorityField">
						<vhmml:input label="Former Owner" listName="formerOwners" listIndex="${loopStatus.index}" fieldName="name" cssClass="contributor" repeatable="true" />
						<vhmml:input label="Former Owner Display" listName="formerOwners" listIndex="${loopStatus.index}" fieldName="displayName" cssClass="displayName"/>
