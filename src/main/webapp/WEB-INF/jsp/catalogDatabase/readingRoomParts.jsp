...
<div class="formSection objectParts">
	...
	<%-- 					<vhmml:input fieldName="cardinalOfOrigin" listName="parts" listIndex="${loopStatus.index}" label="Cardinal of Origin" printedLabel="Cardinal of Printing"/>
				...
				<vhmml:dimension-input label="Support Dimensions" fieldName="supportDimensions" listName="parts" listIndex="${loopStatus.index}" helpText="The outer dimensions of the support measured in centimeters. Round to next closest tenth of a centimeter. Example: 32.2 x 25.7 cm." helpIconClass="catalog-database-icon" cssClass="positiveDecimalsOnly" maxValue="9999.99" importedValue="supportDimensionsImported"/>										
				...
				<div class="formSection partContents contentSection">									
					...
					<c:forEach begin="0" end="${contentCount}" varStatus="contentLoopStatus">
						...
						<div class="contentItem" data-item-id="${contentItem.id}">
							...
							<security:authorize ifNotGranted="ROLE_ADMIN,ROLE_LEVEL_I_CATALOGER,ROLE_LEVEL_II_CATALOGER">
								<vhmml:hidden fieldName="uniformTitle.name" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].uniformTitle.name}"/>
								<vhmml:hidden fieldName="uniformTitle.authorityUriLC" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].uniformTitle.authorityUriLC}"/>
								<vhmml:hidden fieldName="uniformTitle.authorityUriVIAF" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].uniformTitle.authorityUriVIAF}"/>
							</security:authorize>
							...
							<%-- level 2 & 3 catalogers can't edit these fields, but we don't want to clear them out when they save the record so we need to send up the value in these hidden fields --%>
							<security:authorize ifNotGranted="ROLE_ADMIN,ROLE_LEVEL_I_CATALOGER">
								<c:set var="disabled" value="true"/>
								<vhmml:hidden fieldName="catalogerTags" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].catalogerTags}"/>
								<vhmml:hidden fieldName="itemCondition" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].itemCondition}"/>
								<vhmml:hidden fieldName="textBibliography" listName="parts[${loopStatus.index}].contents" listIndex="${contentLoopStatus.index}" fieldValue="${readingRoomObjectForm.parts[loopStatus.index].contents[contentLoopStatus.index].textBibliography}"/>
